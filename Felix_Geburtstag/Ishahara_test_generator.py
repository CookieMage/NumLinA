...
def generate_circle(image_width, image_height, min_diameter, max_diameter):
    # radius = random number in [min_diameter, max_diameter] (the rest is saying that bigger diameters a more probable)
    radius = random.triangular(min_diameter, max_diameter,
                               max_diameter * 0.8 + min_diameter * 0.2) / 2

    # angle = random number in [0,2*pi]
    angle = random.uniform(0, math.pi * 2)
    # distance from center = random number in [0, image_width*0.5-radius]
    distance_from_center = random.uniform(0, image_width * 0.5 - radius)
    x = image_width  * 0.5 + math.cos(angle) * distance_from_center
    y = image_height * 0.5 + math.sin(angle) * distance_from_center

    return x, y, radius

def generate_rectangle(image_width, image_height, min_diameter, max_diameter, weight):
    radius = random.triangular(min_diameter, max_diameter,
                               max_diameter * weight + min_diameter * 1-weight) / 2
...
def generate(relpath = "\\Fran.png", total_circles = TOTAL_CIRCLES, name="ishahara.png", weight = 1):
    image = Image.open(os.getcwd() + "\\" + relpath).convert('RGB')
    ...
    min_diameter = 0.6*weight * math.sqrt((width * height) / (total_circles))
    max_diameter = 1.2*weight * math.sqrt((width * height) / (total_circles))
    
    weight = 0.5
    circle = generate_rectangle(width, height, min_diameter, max_diameter, weight)
    circles = [circle]
    ...
    try:
        for i in range(total_circles):
            weight = (total_circles/(i+1))/total_circles
            tries = 0
            if IMPORTED_SCIPY:
